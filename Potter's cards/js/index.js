...
function validatePassword() {
    ...
    if (passwordRegex.test(document.getElementById("password_inscription").value)) {
        formCorrectInscr += 1
    }
    else {
        passwordError.textContent = "Adresse mail ou mot de passe incorrect, il doit contenir entre 8 et 20 caractères, comporter au minimum une majuscule, une minuscule, un chiffre ainsi qu'un caractère spécial."
    }
}
...
document.getElementById("inscription_submit").addEventListener("click", () => {
    ...
    if (document.getElementById("email_inscription").value != "" && document.getElementById("email_inscription").value.includes("@") && document.getElementById("email_inscription").value.length >= 3) {
        formCorrectInscr++
    }
    if (document.getElementById("pseudo_inscription").value != "") { formCorrectInscr++ }

    if (formCorrectInscr == 3) {
        localStorage.setItem("email", document.getElementById("email_inscription").value)
        localStorage.setItem("pseudo", document.getElementById("pseudo_inscription").value)
        localStorage.setItem("password", document.getElementById("password_inscription").value)
...
document.getElementById("connexion_submit").addEventListener("click", () => {
    ...
    if (document.getElementById("email_connexion").value == localStorage.getItem("email") ||
        document.getElementById("email_connexion").value == localStorage.getItem("pseudo")) {
            formCorrectConn++
        }
    if (document.getElementById("password_connexion").value == localStorage.getItem("password")) {
        formCorrectConn++
    }
    ...
    if (formCorrectConn == 2) {
        ...
        document.getElementById("username_span_index").textContent = localStorage.pseudo
